...
let AccessLevelGuard = class AccessLevelGuard {
    ...
    async canActivate(context) {
        const isPublic = this.reflector.get(key_decorators_1.ACCESS_LEVEL_KEY, context.getHandler());
        if (isPublic) {
            return true;
        }
        ...
        const admin = this.reflector.get(key_decorators_1.ACCESS_LEVEL_KEY, context.getHandler());
        ...
        if (accessLevel === undefined) {
            if (roles === undefined) {
                if (!admin) {
                    ...
                    throw new common_1.UnauthorizedException('No tienes permisos para esta operación');
                }
            }
        }
        ...
        if (userExistInProject === undefined) {
            throw new common_1.UnauthorizedException('No perteneces al proyecto');
        }
